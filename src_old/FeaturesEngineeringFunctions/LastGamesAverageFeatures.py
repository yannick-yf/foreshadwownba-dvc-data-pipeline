...
def LastGamesAverageFeatures(TRAINING_DF):
    ...
    for col in colums_to_process:
        ...
        TRAINING_DF['before_average_' + col ] = round(TRAINING_DF.groupby(['id_season', 'tm'])[col].transform(lambda x: x.shift(1).expanding().mean()), 1)
        TRAINING_DF['before_average_lastfivegame_' + col ] = round(TRAINING_DF.groupby(['id_season', 'tm'])[col].transform(lambda x: x.shift(1).rolling(5).mean()), 1)
        TRAINING_DF['before_average_lasttengame_' + col ] = round(TRAINING_DF.groupby(['id_season', 'tm'])[col].transform(lambda x: x.shift(1).rolling(10).mean()), 1)

        # Fill na values
        TRAINING_DF['before_average_lastfivegame_' + col ] = TRAINING_DF['before_average_lastfivegame_' + col ].fillna(TRAINING_DF['before_average_' + col ])
        TRAINING_DF['before_average_lasttengame_' + col ] = TRAINING_DF['before_average_lasttengame_' + col ].fillna(TRAINING_DF['before_average_lastfivegame_' + col ])
        TRAINING_DF['before_average_lasttengame_' + col ] = TRAINING_DF['before_average_lasttengame_' + col ].fillna(TRAINING_DF['before_average_' + col])

        TRAINING_DF['diff_all_minus_lastfivegame_' + col ] = TRAINING_DF['before_average_' + col ] - TRAINING_DF['before_average_lastfivegame_' + col ]
        TRAINING_DF['diff_all_minus_lasttengame_' + col ] = TRAINING_DF['before_average_' + col ] - TRAINING_DF['before_average_lasttengame_' + col ]

    return TRAINING_DF


def LastGamesRatioAverageFeatures(TRAINING_DF):

    df = TRAINING_DF[['id_season', 'game_nb', 'game_date', 'extdom', 'tm', 'opp', 'results']]
...
def LastSeasonRatioFeatures(TRAINING_DF):
    ...
    LastSeasonRatioFeatures_df = TRAINING_DF[TRAINING_DF['game_nb']==TRAINING_DF['max_game_id']]

    LastSeasonRatioFeatures_df['before_season_ratio'] = LastSeasonRatioFeatures_df['w_tot'] / LastSeasonRatioFeatures_df['max_game_id']
    LastSeasonRatioFeatures_df['id_season_plus1'] = LastSeasonRatioFeatures_df['id_season'] + 1

    LastSeasonRatioFeatures_df = LastSeasonRatioFeatures_df[['id_season_plus1', 'tm', 'before_season_ratio']]
